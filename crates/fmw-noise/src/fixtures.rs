...
fn places_every_vulcanus_cliff_where_the_game_places_it() {
    ...
    assert_eq!(ship, row(1569, 1541, 1530, 1521), "shipping totals");
    ...
    // (876 -> 871) and ONE true cliff with them. Re-frozen again when the rule
    // started reading the game's own roll (#84): one more surplus cell goes
    // (871 -> 870) and one wrong orientation is fixed, and that one true cliff
    // is still lost. Before #414 the rule cost no true positive at all, and
    // removed 23.
    assert_eq!(
        lava.matched - ship.matched,
        1,
        "true positives the ore rejection costs"
    );
    assert_eq!(lava.ours - ship.ours, 29, "cells the ore rejection removed");
    assert_eq!(
        (
...
fn vulcanus_cliffs_track_the_volcanism_sliders() {
    ...
    // removal (`cliffs/vulcanus_ore_rejection.rs`) - and 1521/9/11/1 once the
    // rule read the game's own roll (#84).
    ...
    assert_eq!(rows[0].2, row(1521, 9, 11, 1, 38), "default arm totals");
    ...
    // 7 -> 9 by arm.
    //
    // Re-frozen when the rule started reading the game's own roll (#84):
    // default R2 850/10/11/1 -> 851/9/10/1, size 3 R2 534/4/5/0 -> 533/3/4/2,
    // the rest unchanged. Errors 23 -> 21 at the default and 9 -> 9 at size
    // 3, where the roll trades a wrong and a surplus for two missing.
    //
...
fn the_volcanism_contrast_out_of_sample() {
    ...
    //
    // Re-frozen when the rule started reading the game's own roll (#84): the
    // default arm 4505/37/55/3 -> 4508/33/50/4 (95 -> 87 errors) and the
    // frequency-0.5 arm 4006/48/68/10 -> 4007/47/62/10 (126 -> 119). No region
    // got worse; the largest single move is default region 3, 14 -> 9.
    let row = |matched, wrong, surplus, missing, unscored| SweepRow {
        matched,
        wrong,
        surplus,
    ...
    assert_eq!((errors(d), comparable(d)), (87, 4545), "default: 1.91%");
    assert_eq!(
    ...
    assert!(
        (-3.10..-3.00).contains(&z),
        "z = {z:.3}: measured -3.07 (-2.96 before the roll, -2.55 before #414, -1.80 before #407), and the SIGN is the finding"
    );

    // What is real is the spread BETWEEN regions, at one slider setting: the
    // frequency 0.5 arm's `[-2200,-1500]` carries 66 of that arm's 119 errors
    // on its own - 23 wrong, 40 surplus and 3 missing of 795 comparable cells,
    // 8.3% (67 of 126 before the roll, 70 of 124 before #414, 74 of 138
    // before #407) - while
...
fn the_concentrated_residual_against_the_ore_lever() {
    ...
    //
    // Re-frozen when the rule started reading the game's own roll (#84): the
    // port's rule removes 44, one fewer, and that cell is one of the two
    // ring `missing` back again, so the ON row is 769/23/40/3. The 24 surplus
    // cells that are the game's removals do not move: 21 are geyser removals
    // (`the_ore_field_where_the_cliff_rule_reads_it_at_frequency_half` bins
    // them).
    ...
    assert_eq!(row_on, row(769, 23, 40, 3), "resources ON, port with ore");
    assert_eq!(
        row_no_ore,
        row(768, 27, 81, 0),
    ...
    assert_eq!(port_removed.len(), 44, "cliffs the port's ore rule removed");
    assert_eq!(
        surplus_is_ore, 24,
...
fn the_ore_field_where_the_cliff_rule_reads_it_at_frequency_half() {
    ...
    let ring: Vec<(&str, f64, usize, usize)> = [
        ...
        ("rp = 0.95, the midpoint the rule read before the roll", 53.2),
    ...
    //
    // Re-frozen when the rule started reading the game's own roll (#84): the
    // rule's footprint is now exactly the game's, 1,190 tiles and no other,
    // and the ring table above stays as the record of why. The 24 disputed
    // cells do not move at all. So the remaining ore-rule surplus here is the
    // geyser, plus one cell half a tile from a calcite the port now places
    // correctly - that one is geometry, not the field.
    assert_eq!(
        looked.len(),
        11362,
    ...
    assert_eq!(game_only.len(), 0, "game ore tiles the rule's field lacks");
    assert_eq!(port_only.len(), 0, "rule field tiles with no game entity");
    assert_eq!(port_only_at_one, 0, "of those, touching a game ore tile");
    assert_eq!(
        disputed.len(),
        24,
...
fn the_apply_stage_beats_the_crossing_stage_on_three_counts_and_loses_on_none() {
    ...
    //
    // Re-frozen when the rule started reading the game's own roll (#84):
    // 1520/10/12/1 -> 1521/9/11/1, 1523/7/11/1 -> 1524/6/10/1,
    // 1511/19/12/1 -> 1512/18/11/1. The one `missing` stays.
    assert_eq!(
        totals[0],
        row(1521, 9, 11, 1),
        "rejectAtCrossingStage (ships)"
    );
    assert_eq!(totals[1], row(1524, 6, 10, 1), "applyCliffs, lava + ore");
    assert_eq!(totals[2], row(1512, 18, 11, 1), "applyCliffs, no cascade");
    ...
    assert_eq!(
        ...
        "both zero since #407, both one since #414"
    );
...
// ---------------------------------------------------------------------------
// #84 - `Surface::wouldCollide`'s OWN decisions, read out of the running game.
//
// `oracle-vulcanus-wouldcollide` is not a Lua capture. It is every call the
// 2.0.77 map generator made to the apply-stage collision test while creating
// the three `oracle-vulcanus-cliff-entities` regions, recorded by an lldb
// breakpoint script (`scripts/probes/vulcanus-cliff-wouldcollide`). So for the
// first time the port's predicate is graded against the game's predicate
// directly, one boolean per queued cliff, rather than against the cliffs that
// survived it.
// ---------------------------------------------------------------------------

/// One recorded `Surface::wouldCollide(CliffPrototype const&, MapPosition
/// const&, CliffOrientation)` call. Boxes are in `MapPosition`'s 1/256 units.
struct WouldCollideCall {
    /// `cliff-vulcanus`, or `crater-cliff` for a crater ring segment.
    prototype: String,
    x: f64,
    y: f64,
    orientation: u8,
    /// The orientation's `collision_bounding_box` with the position added.
    boxed: [i32; 4],
    /// The box's orientation word: `[sin, -cos]` in 1.15 fixed point.
    rotation: [i32; 2],
    /// What `BoundingBox::getAABB` returned for `boxed`.
    aabb: [i32; 4],
    /// What `Surface::checkTileCollisions` returned.
    tile_hit: bool,
    /// The vtable symbol of the entity `Surface::collideWithEntity` returned.
    entity: Option<String>,
    /// The call's return: `true` destroys the cliff.
    result: bool,
}

fn would_collide_calls(fixture: &Json) -> Vec<(i64, i64, Vec<WouldCollideCall>)> {
    let four = |v: &Json| -> [i32; 4] {
        let a = v.as_f64_array();
        #[allow(clippy::cast_possible_truncation)]
    ...
    fixture
        ...
        .map(|case| {
            let r = case.get("region");
            #[allow(clippy::cast_possible_truncation)]
            let (x0, y0) = (r.get("x0").as_f64() as i64, r.get("y0").as_f64() as i64);
            let calls = case
                .get("calls")
                .as_array()
                .iter()
                .map(|c| {
                    let rot = c.get("rotation").as_f64_array();
                    #[allow(clippy::cast_possible_truncation)]
                    WouldCollideCall {
                        prototype: c.get("prototype").as_str().to_string(),
                        x: c.get("x").as_f64(),
                        y: c.get("y").as_f64(),
                        orientation: c.get("orientation").as_f64() as u8,
                        boxed: four(c.get("box")),
                        rotation: [rot[0] as i32, rot[1] as i32],
                        aabb: four(c.get("aabb")),
                        tile_hit: c.get("tileHit").as_bool(),
                        entity: match c.get("entity") {
                            Json::Null => None,
                            e => Some(e.get("kind").as_str().to_string()),
                        },
                        result: c.get("result").as_bool(),
                    }
                })
                .collect();
            (x0, y0, calls)
...
#[test]
fn the_aabb_derivation_reproduces_get_aabb_on_every_recorded_call() {
    let fixture = load_captured_at(
        "test/fixtures/oracle-vulcanus-wouldcollide.seed123456.json",
        "2.0.77",
    );
    let mut calls = 0;
    ...
    assert_eq!(calls, 2157);
}

/// The port's shipped tile half: the RAW stored rectangle over an inclusive
/// floor, the `#90` model.
fn port_tile_collides(orientation: u8, x: f64, y: f64, lava: &dyn TileCollision) -> bool {
    let code = cliff_code_for_orientation(orientation).expect("a placing orientation");
    let b = cliff_collision_tile_box(code, x, y).expect("a box");
    for tx in b.left..=b.right {
        for ty in b.top..=b.bottom {
            if lava.collides(tx, ty) {
                return true;
            }
        }
    }
...
struct BooleanScore {
    agree: usize,
    /// The model says collide, the game said not.
    false_hit: usize,
    /// The game said collide, the model says not.
    missed_hit: usize,
}

impl BooleanScore {
    fn add(&mut self, model: bool, game: bool) {
        match (model, game) {
            (true, false) => self.false_hit += 1,
            (false, true) => self.missed_hit += 1,
            _ => self.agree += 1,
        }
    }
}
...
fn the_games_apply_stage_box_is_the_catalogs_box_with_the_rotbb_tag_kept() {
    ...
    for (_, _, region) in would_collide_calls(&fixture) {
        for c in &region {
            ...
            rotated += usize::from(is_rotated);
            aabb_widened += usize::from(c.aabb != c.boxed);
            assert_eq!(
                is_rotated,
                c.aabb != c.boxed,
                "getAABB changes the box exactly when the sin word is set, at ({}, {})",
                c.x,
                c.y
            );
        }
...
/// 2157 calls - by the oriented test, and the shipped raw rectangle disagrees
/// on 19.** Graded one call at a time against what the game's own
/// `checkTileCollisions` returned, with the port's lava tiles under both
/// models, so the lava resolver is held constant and only the geometry moves.
///
...
///
/// Seventeen of the nineteen are rotated `cliff-vulcanus` boxes, where the raw
/// rectangle reaches tiles the 45-degree rectangle does not and misses tiles it
/// does; the other two are crater segments, whose prototype the shipped
/// catalog does not hold at all. The six false rejections `#84` carried since
/// UPDATE 6 - `86,38.5`, `22,178.5`, `1638,1598.5`, `1638,1602.5`,
/// `1662,1634.5`, `-1054,1018.5` - are all among the eight "says collide".
///
/// The entity half is counted here and modelled nowhere: 35 kills, 23 of them
/// crater segments or cells against another cliff, 7 against a rock and 5
/// against a demolisher body segment. `result` is exactly `tile OR entity` on
/// every call, so there is no third mechanism inside `wouldCollide`.
#[test]
fn the_tile_half_of_would_collide_graded_against_the_games_own_calls() {
    let fixture = load_captured_at(
        "test/fixtures/oracle-vulcanus-wouldcollide.seed123456.json",
        "2.0.77",
    );
    ...
    let mut shipped = BooleanScore::default();
    let mut oriented = BooleanScore::default();
    let mut shipped_rot = BooleanScore::default();
    let mut oriented_rot = BooleanScore::default();
    let mut entity_kills: BTreeMap<String, usize> = BTreeMap::new();
    let mut tile_kills = 0;
    ...
    for (_, _, region) in would_collide_calls(&fixture) {
        for c in &region {
            ...
            if c.rotation[0] & 0xffff != 0 {
                shipped_rot.add(s, c.tile_hit);
                oriented_rot.add(o, c.tile_hit);
            }
            if s != c.tile_hit && c.prototype == "cliff-vulcanus" {
                disagreements.push((c.x, c.y));
            }
            if c.tile_hit {
                tile_kills += 1;
            }
            if let Some(kind) = &c.entity {
                *entity_kills.entry(kind.clone()).or_default() += 1;
            }
            assert_eq!(
                c.result,
                c.tile_hit || c.entity.is_some(),
                "result is tile OR entity"
            );
        }
    }
    let score = |agree, false_hit, missed_hit| BooleanScore {
        agree,
        false_hit,
        missed_hit,
    };
    assert_eq!(oriented, score(2157, 0, 0), "oriented, every call");
    assert_eq!(oriented_rot, score(1420, 0, 0), "oriented, rotated boxes");
    assert_eq!(
        shipped,
        score(2138, 8, 11),
        "shipped raw rectangle, every call"
    );
    assert_eq!(
        shipped_rot,
        score(1403, 8, 9),
        "shipped raw rectangle, rotated boxes"
    );
    assert_eq!(tile_kills, 195);
    assert_eq!(
        entity_kills,
        BTreeMap::from([
            ("Cliff".to_string(), 23),
            ("Segment".to_string(), 5),
            ("SimpleEntity".to_string(), 7),
        ])
    );
    // UPDATE 6's six false rejections are all shipped-model disagreements.
    for cell in [
        (86.0, 38.5),
        (22.0, 178.5),
        (1638.0, 1598.5),
        ...
        assert!(
            disagreements.contains(&cell),
            "{cell:?} is a shipped disagreement"
        );
    }
    assert_eq!(
        disagreements.len(),
        17,
        "shipped disagreements on cliff-vulcanus"
    );
}
...
impl ApplyCollision for OrientedLavaAndOre<'_, '_> {
    fn collides(&self, orientation: u8, x: f64, y: f64) -> bool {
        if self
            .entity_kills
            .is_some_and(|k| k.contains(&(x.to_bits(), y.to_bits())))
        {
            return true;
        }
        tile_collides(orientation, x, y, &self.lava) || self.ore.removes(orientation, x, y)
...
/// **The oriented tile test takes `missing` to ZERO through the apply stage**,
/// scored on the game's 1531 cliffs exactly as
/// [`the_apply_stage_beats_the_crossing_stage_on_three_counts_and_loses_on_none`]
/// scores the shipped model:
///
/// | arm | matched | wrong | surplus | missing |
/// | --- | ---: | ---: | ---: | ---: |
/// | shipped `LavaAndOre` | 1508 | 18 | 22 | 5 |
...
///
/// Of the 16 cells the third arm still gets wrong, every one is in
/// `[1500,1500]`. Fourteen are cells the game's `wouldCollide` KEPT -
/// `result = false` in the fixture - that are nonetheless absent from the
/// game's map, so they die after the apply stage, not in it: the `1542/1546`
/// knot and `1622/1626` pair UPDATE 8 tied to the ore lever, the
/// `1606`/`1630` cells beside them, and the `1742/1746` run that sits under a
/// demolisher body segment's box (`Segment`, 1745.2..1753.0 x 1532.1..1538.3).
/// No collision box can reach a cell the collision test passed; that residual
/// belongs to the ore rule's geometry and to whatever places the demolisher.
///
/// The other two, `(1626, 1602.5)` and `(1630, 1602.5)`, the game KILLED on
/// lava - with the raw queued orientation. `apply_cliff_connections` tests a
/// cell after earlier chunks' destructions have cascaded into it, so it tested
/// those two with a trimmed box that misses the lava. The fourth arm hands the
/// pass the raw orientation by position and takes exactly those two.
///
...
///
/// Re-frozen when the ore rule started reading the game's own roll (#84):
/// 1505/19/17/7 -> 1506/18/16/7, 1523/7/11/1 -> 1524/6/10/1,
/// 1526/4/8/1 -> 1527/3/7/1, leftovers 13 -> 11. The one `missing` stays.
#[test]
fn the_oriented_tile_test_through_the_apply_stage() {
    ...
    let calls = load_captured_at(
        "test/fixtures/oracle-vulcanus-wouldcollide.seed123456.json",
        "2.0.77",
    );
    let mut entity_kills: BTreeSet<(u64, u64)> = BTreeSet::new();
    for (_, _, region) in would_collide_calls(&calls) {
        for c in &region {
            if c.entity.is_some() && c.prototype == "cliff-vulcanus" {
    ...
    let oriented = OrientedLavaAndOre {
        lava: VulcanusLavaTiles::new(&stack),
        ore: VulcanusOreRejection::new(&stack, &ctx.vulcanus_resource_controls),
        entity_kills: None,
    };
    ...
    for case in fixture.get("cases").as_array() {
        ...
        for (i, apply) in arms.iter().enumerate() {
            let port: BTreeMap<(u64, u64), u8> = apply_cliff_connections(
                &raw,
                &CliffConnectionOptions {
                    collides: Some(*apply),
            ...
            for (k, id) in &port {
                match game.get(k) {
                    None => {
                        totals[i].surplus += 1;
                        if i == 2 {
                            leftovers.push((f64::from_bits(k.0), f64::from_bits(k.1)));
                        }
                    }
                    Some(want) if want == id => totals[i].matched += 1,
                    Some(_) => {
    ...
    assert_eq!(totals[1], row(1524, 6, 10, 1), "oriented tile test + ore");
    assert_eq!(
        totals[2],
        row(1527, 3, 7, 1),
    ...
    for (x, y) in &leftovers {
        assert!(
            (1500.0..1756.0).contains(x) && (1500.0..1756.0).contains(y),
            "({x}, {y}) in [1500,1500]"
        );
        let (o, result) = tested
            .get(&(x.to_bits(), y.to_bits()))
            .unwrap_or_else(|| panic!("the game queued ({x}, {y})"));
        if *result {
            killed_by_game.push((*x, *y, CLIFF_ORIENTATION_NAMES[usize::from(*o)]));
        } else {
            kept_by_game += 1;
        }
    }
    assert_eq!(kept_by_game, 11);
    assert_eq!(
        killed_by_game,
        Vec::<(f64, f64, &str)>::new(),
...
/// the orientation `CliffPlacement` computes for that cell, with no cell
/// queued by one side and not the other inside the regions.** UPDATE 6
/// established the queue's exactness through the cascade; this is the queue
/// before anything touches it.
///
/// That settles which orientation the apply pass must hand to `collides`.
/// `apply_cliff_connections` tests a cell AFTER earlier destructions have
/// cascaded into it, so a cell whose neighbour died first is tested with a
/// trimmed box; the game tests the queued box. `(1626, 1602.5)` and
/// `(1630, 1602.5)` are the two cells that shows on, and the last arm here
/// hands the pass the raw orientation by position to measure what fixing it
/// is worth.
#[test]
fn the_game_tests_the_raw_queued_orientation_and_the_raw_queue_is_exact() {
    let fixture = load_captured_at(
        "test/fixtures/oracle-vulcanus-cliff-entities.seed123456.json",
        "2.1.12",
    );
    ...
    let tested: BTreeMap<(u64, u64), u8> = would_collide_calls(&calls)
        .iter()
        .flat_map(|(_, _, region)| region.iter())
    ...
    for case in fixture.get("cases").as_array() {
        ...
        let raw_by: BTreeMap<(u64, u64), u8> = raw
            .iter()
            .filter_map(|c| {
                cliff_orientation_for_code(c.code).map(|id| ((c.x.to_bits(), c.y.to_bits()), id))
            })
            .collect();
        // The game's calls reach one chunk past the region; compare where the
        // port's 64-tile halo certainly covers them.
        let inside =
            |x: f64, y: f64| x >= x0 - 32.0 && x < x1 + 32.0 && y >= y0 - 32.0 && y < y1 + 32.0;
        for (k, o) in &tested {
            let (x, y) = (f64::from_bits(k.0), f64::from_bits(k.1));
            if !inside(x, y) {
                continue;
            ...
            match raw_by.get(k) {
                None => game_only += 1,
                Some(id) if id == o => same += 1,
                Some(_) => differ += 1,
            }
        }
        for k in raw_by.keys() {
            let (x, y) = (f64::from_bits(k.0), f64::from_bits(k.1));
            if x >= x0 && x < x1 && y >= y0 && y < y1 && !tested.contains_key(k) {
                port_only += 1;
            }
        }
    }
    assert_eq!((same, differ, game_only, port_only), (2088, 0, 0, 0));
}
...
fn the_removal_box_is_the_resources_tile_widened_aabb_against_the_cliffs_oriented_box() {
    ...
    // rows now carry that geometry over the PORT's ore field, read through
    // the game's own roll (#84) - 79 and 86 errors, 81 and 89 at the removal
    // threshold the rule read before - and the gap that remains to the
    // game-entity rows is the absent geyser, measured in
    // `cliffs/vulcanus_ore_rejection.rs`. The base box's
    ...
    assert_eq!(f05[2], row(769, 23, 40, 3), "f0.5, crossing: shipped");
    assert_eq!(
        f05[3],
        row(776, 16, 19, 3),
    ...
    assert_eq!(r1500[0], row(854, 6, 10, 1), "[1500,1500], apply: shipped");
    assert_eq!(
        r1500[1],
        row(855, 5, 6, 1),
        "[1500,1500], apply: engine geometry"
    );
    assert_eq!(totals[0], row(1625, 26, 48, 5), "apply: shipped");
    assert_eq!(totals[1], row(1634, 17, 22, 5), "apply: engine geometry");
    assert_eq!(totals[2], row(1620, 32, 50, 4), "crossing: shipped");
    ...
    //
    // With the port's OWN ore field (the first table), the extra ore-bin
    // errors are the sulfuric-acid geyser the port leaves out. They were the
    // field's boundary ring as well until the rule read the game's own roll
    // (#84), which took `[1500,1500]`'s bin from 7 to 5 and left f0.5's 35.
    ...
    assert_eq!(
        bins[0][0],
        table(&[("wouldCollide Segment", 10), ("postSetup", 5), (KEPT, 2)]),
        "[1500,1500], shipped"
    );
    assert_eq!(
        bins[0][1],
...
/// The Vulcanus solid-ore roll against the game's own ore entities (#84).
///
/// Every solid-ore probability carries `random_penalty_between(0.9, 1, 1)`,
/// and [`VulcanusOreRoll`](crate::resources::vulcanus_ore_roll::VulcanusOreRoll)
/// takes its batch to be the tile's chunk, row-major, at integer tile
/// coordinates. This grades [`VulcanusOreFootprint::occupies`] - the shipped
/// code path - against every `tungsten-ore`, `coal` and `calcite` entity in four
/// regions: the three default-preset regions of
/// `oracle-vulcanus-resource-entities` (2.1.12) and the volcanism frequency-0.5
/// region of `oracle-vulcanus-cliff-volcanism-ore` (2.1.17). Different field
/// settings and different game builds, so this is not one region fitted twice.
///
/// **The window is every tile within four of a game ore tile**, not the whole
/// region, for cost: the whole-region run was 49 s in a debug build and the
/// poison run pays about four times that. It contains what it grades. Graded
/// over all 262,144 tiles on 2026-09-26, the `rp = 1` footprint painted 81,
/// 398, 119 and 212 tiles the game left bare, and the roll 0, 1, 1 and 0 - the
/// same numbers this window freezes below. So nothing either arm places lies
/// outside it.
///
/// Four controls, each computed here rather than through the shipped code:
/// the two thresholds the port used before the roll (`rp = 1` for the overlay,
/// the 0.95 midpoint for the cliff rule), and the same chunk batch walked
/// COLUMN-major or seeded at tile CENTRES. Column-major loses in every region.
/// The centre control can only lose where a coordinate is negative - see the
/// relations at the end - and there it loses by 90 and 133 tiles.
#[test]
fn the_vulcanus_ore_roll_is_the_chunk_batch() {
    use crate::random_penalty::{random_penalty_batch, RandomPenaltyParams, RandomPenaltyPosition};
    use crate::resources::vulcanus_ore_roll::ore_probability;
    const SOLID_ORES: [&str; 3] = ["tungsten-ore", "coal", "calcite"];
    const REACH: i64 = 4;

    let entities = load_captured_at(
        "test/fixtures/oracle-vulcanus-resource-entities.seed123456.json",
        "2.1.12",
    );
    let volcanism = load_captured_at(
        "test/fixtures/oracle-vulcanus-cliff-volcanism-ore.seed123456.json",
        "2.1.17",
    );
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let seed0 = entities.get("seed").as_f64() as u32;
    let mut f05 = crate::eval::ctx::EvalCtx::new(seed0);
    f05.vulcanus_volcanism_frequency = 0.5;
    let mut runs: Vec<(Json, crate::eval::ctx::EvalCtx)> = entities
        .get("cases")
        .as_array()
        .iter()
        .map(|c| (c.clone(), crate::eval::ctx::EvalCtx::new(seed0)))
        .collect();
    runs.push((
        volcanism.get("arms").as_array()[0].get("cases").as_array()[0].clone(),
        f05,
    ));

    // The same chunk batch under a different layout, for the two controls.
    let control_penalty = |tx: i64, ty: i64, centre: bool, row_major: bool| -> f64 {
        let (cx, cy) = (tx.div_euclid(32), ty.div_euclid(32));
        let offset = if centre { 0.5 } else { 0.0 };
        #[allow(clippy::cast_precision_loss)]
        let positions: Vec<RandomPenaltyPosition> = (0..1024_i64)
            .map(|i| {
                let (dx, dy) = if row_major {
                    (i % 32, i / 32)
                } else {
                    (i / 32, i % 32)
                };
                RandomPenaltyPosition {
                    x: (cx * 32 + dx) as f64 + offset,
                    y: (cy * 32 + dy) as f64 + offset,
                }
            })
            .collect();
        let batch = random_penalty_batch(
            &positions,
            &[1.0; 1024],
            &RandomPenaltyParams {
                seed: 1.0,
    ...
    // Per region: game ore tiles, then (game only, port only) for the roll,
    // `rp = 1`, the midpoint, centre coordinates and column-major.
    let mut measured: Vec<(usize, [(usize, usize); 5])> = Vec::new();
    for (case, ctx) in &runs {
        let r = case.get("region");
        #[allow(clippy::cast_possible_truncation)]
        let (x0, y0, x1, y1) = (
            r.get("x0").as_f64() as i64,
            r.get("y0").as_f64() as i64,
            r.get("x1").as_f64() as i64,
            r.get("y1").as_f64() as i64,
        );
        let inside = |(tx, ty): (i64, i64)| tx >= x0 && tx < x1 && ty >= y0 && ty < y1;
        let game: BTreeSet<(i64, i64)> = case
            .get("resources")
            .as_array()
            .iter()
            .filter(|e| SOLID_ORES.contains(&e.get("name").as_str()))
            .map(|e| {
                #[allow(clippy::cast_possible_truncation)]
                let t = (
                    e.get("x").as_f64().floor() as i64,
            ...
            .filter(|&t| inside(t))
            .collect();
        let mut window: BTreeSet<(i64, i64)> = BTreeSet::new();
        for &(gx, gy) in &game {
            for ty in gy - REACH..=gy + REACH {
                for tx in gx - REACH..=gx + REACH {
                    if inside((tx, ty)) {
                        window.insert((tx, ty));
                    }
                }
        ...
        let stack = VulcanusStack::with_host_trig(&base, &biomes);
        let footprint = VulcanusOreFootprint::new(&ctx.vulcanus_resource_controls);
        let mut arms = [(0usize, 0usize); 5];
        for &(tx, ty) in &window {
            let score = footprint.score(&stack, tx, ty);
            let rolled = |rp: f64| ore_probability(score / 1000.0, rp) >= 0.5;
            let placed = [
                footprint.occupies(&stack, tx, ty),
                score >= 0.5,
                score >= 53.2,
                rolled(control_penalty(tx, ty, true, true)),
                rolled(control_penalty(tx, ty, false, false)),
            ];
            let is_game = game.contains(&(tx, ty));
            for (arm, port) in arms.iter_mut().zip(placed) {
                match (port, is_game) {
                    (false, true) => arm.0 += 1,
                    (true, false) => arm.1 += 1,
                    _ => {}
        ...
        eprintln!(
            "[{x0},{y0}]: game {}, window {}; roll / rp=1 / midpoint / centre / column-major \
             (game only, port only): {arms:?}",
            game.len(),
            window.len()
        );
        measured.push((game.len(), arms));
    }

    // The frozen finding, measured 2026-09-26. Read a moved number, do not
    // adjust it. The roll misses none of the 7,096 game tiles and adds two,
    // both in the band where the probability is between 0 and 1, where the
    // game rolls a second time and a threshold is the best a pure function
    // can do.
    assert_eq!(
        measured,
        vec![
            (945, [(0, 0), (0, 81), (17, 24), (0, 0), (25, 28)]),
            (3914, [(0, 1), (0, 398), (99, 100), (0, 1), (131, 137)]),
            (1047, [(0, 1), (0, 119), (33, 24), (57, 33), (40, 41)]),
            (1190, [(0, 0), (0, 212), (43, 58), (62, 71), (69, 75)]),
        ]
    );
    // Stated as relations too: the roll misses nothing and beats every
    // control, with one exception that is arithmetic rather than luck. The
    // batch is seeded from its first position TRUNCATED toward zero, so
    // moving it half a tile changes nothing where both coordinates are
    // non-negative: the centre control IS the roll in `[0,0]` and
    // `[1500,1500]`, and only the two regions with a negative coordinate can
    // tell the two apart.
    let errors = |a: (usize, usize)| a.0 + a.1;
    for (i, (_, arms)) in measured.iter().enumerate() {
        assert_eq!(arms[0].0, 0, "the roll misses no game ore tile");
        for control in [arms[1], arms[2], arms[4]] {
            assert!(errors(arms[0]) < errors(control));
        }
        if i < 2 {
            assert_eq!(arms[3], arms[0], "centre = integer where x, y >= 0");
        } else {
            assert!(errors(arms[0]) < errors(arms[3]));
        }
    }
}
