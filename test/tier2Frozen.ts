 ...
 *
 * **The table plumbing lives in `frozenTable.ts`**, shared with the tier-3
 * render freeze, which needs the same guards for the same reasons. What is
 * tier-2 specific is the file, the environment variable, and the reasoning
 * above.
 *
 * ## Recording
 *
...
const TABLE = makeFrozenTable({
  tablePath: join(import.meta.dirname, "fixtures", "tier2-checksums.json"),
  envVar: "FMW_FREEZE_TIER2",
});
...
/** Record one checksum under record mode. */
export const record = TABLE.record;
